...
my $rnd_cycle = POSIX::round($cycletime * 5) / 10;
...
foreach my $s (keys %sources) {
	...
	while ($finished == 0) {
		...
		my $mset = &measure($s, $sources{$s},
				    ...
				    $iter_starttime, $iter_endtime, $rc);
		print Dumper $mset;
		if ($#{$mset->{'fluxDensityData'}->{'data'}} > -1) {
		    # Copy the required parameters.
		    my $tso = { 'fluxDensityData' => $mset->{'fluxDensityData'},
				'flaggedFraction' => $mset->{'flaggedFraction'},
				'defect' => $mset->{'defect'},
...
sub calibrate {
	...
	if ($datasets[$i] =~ /^.*\.(.*)\/visdata$/) {
		...
		my $c = ($fconfig->[$j]->[1] + ($fconfig->[$j]->[0] - 1) *
			 $fconfig->[$j]->[2] / 2) * 1000;
		print "c = $c  f = $f\n";
		if (abs($c - $f) < abs($fconfig->[$j]->[2] * 1000 / 2)) {
		    $fcfg = $j;
		    last;
		}
	    }
	...
	if ($npoints > 0) {
		...
		"options=xyvary,qusolve,nopol nfbin=2 refant=3";
	    print $gpcal_cmd."\n";
	    system $gpcal_cmd;
